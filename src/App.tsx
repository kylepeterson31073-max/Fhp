...
export const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 flex flex-col font-sans selection:bg-indigo-500 selection:text-white">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-3 sm:p-5 lg:p-6 flex flex-col">
        ...
        {activeTab === 'map' && (
          <div className="flex-1 flex flex-col space-y-3">
            ...
            {/* Split View: Map (Left/Top) + Cards Feed (Right/Bottom) */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 flex-1 min-h-[600px]">
              
              {/* Interactive Leaflet Map Column */}
              <div className="lg:col-span-7 h-[460px] lg:h-full relative rounded-2xl overflow-hidden border border-slate-200 shadow-xs bg-slate-100">
                <MapView
              ...
              <div className="lg:col-span-5 flex flex-col space-y-2.5 max-h-[700px] overflow-y-auto pr-1">
      ...
      {activeTab !== 'advisors' && (
        <div className="fixed bottom-12 right-5 z-40">
          <button
            onClick={() => setActiveTab('advisors')}
            className="px-3.5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-lg border border-indigo-400/30 flex items-center gap-2.5 transition transform hover:scale-105"
            title="Chat with Daisy & AI Specialists"
          >
            <DaisyMascotBadge size="sm" animate={true} />
            <div className="text-left">
              <span className="block text-xs font-bold leading-none">Ask Daisy</span>
              <span className="text-[10px] text-indigo-200 leading-none">6 AI Specialists Ready</span>
            </div>
          </button>
        </div>
      )}
      ...
      <footer className="h-8 bg-indigo-900 text-white flex items-center px-4 justify-between text-[10px] font-medium shrink-0 z-30">
        <div className="flex items-center gap-3">
